# 002: (Take 1) Topical Factional Liquid Democracy
...
# 002: (Take 2) Topical, Factional Liquid Democracy
...
Even when institutions start out democratic, they have a tendency to collapse into authoritarian rule over time: examples include ancient Athens, Rome, post cold-war Russia, perhaps the USA given the Jan 7th riots if they had succeeded in installing Trump as America's fist dictator,
...
It is clear then that while the instant redelegation ability of liquid democracy should be kept, we need to edit this such that someone can delegate to others based on topics. Such that I can delegate to person A for energy infrastructure, person B for monetary policy, person C for foreign policy, etc.
The last problem that then remains with this topical liquid democracy is that delegation is going to a single person who may end up holding a large amount of power and thus can be corruptible.  
...
This nice property about this is we suddenly have a system for delegating decision making from the most high level topics all the way down to the most minute of details. 
